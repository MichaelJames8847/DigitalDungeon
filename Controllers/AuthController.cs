...
namespace DigitalDungeon.Controllers;
...
public class AuthController : ControllerBase
{
    ...
    public AuthController(DigitalDungeonDbContext context, UserManager<IdentityUser> userManager)
    {
        _dbContext = context;
    ...
    // Endpoint from the AuthController that gets the userProfile for a logged in user (common use case for most front-end apps)
    // [Authorize] attribute on this method tells the framework to require a cookie in order to access this resource.
    // A request without a cookie will get an automatic 401 response.
    // User property is inherited from ControllerBase, and contains all of the data from the cookie (each of the "claims").
    // Method is getting userId (to look up user's UserProfile) and all of the roles for that user
    [HttpGet("Me")]
    [Authorize]
